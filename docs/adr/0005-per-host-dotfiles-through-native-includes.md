...
**The seam has one real side.** Only `tower` carries confirmed values. The `laptop` files exist and are deliberately near-empty rather than filled with invented connector names — per ADR-0003 that host builds green and cannot boot, so any monitor layout written for it would be fiction that `nix flake check` would happily bless. The check proves the split evaluates for both hosts, not that the laptop's half is right.

**A host's keyboard layout is now stated in three places** — `console.keyMap`, the greeter's `keyboard.layout`, and the niri per-host file — because they are three different naming schemes for it. `console.keyMap = "uk"` is a kbd keymap name and is not a valid XKB layout, where the same layout is `gb`, so no one of them can be derived from another.

**Only files that are linked remain in `.dotfiles`.** Runtime output — the snippets noctalia regenerates, the settings the UI writes — is no longer linked anywhere, so the tree is now readable as "these are the files a human writes", with `starship.toml` the single documented exception.
