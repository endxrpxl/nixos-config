...
> **Amended by ADR-0009.** This document's security objection to hibernation — the paragraph starting "The kernel is right to refuse" — no longer holds: the root filesystem is now LUKS-encrypted (ADR-0009), so a hibernation image would no longer sit in plaintext. The *decision* is unchanged. The kernel's `hibernation_available()` gate does not ask whether the disk is encrypted, and `secretmem_active()` still reads true while Bitwarden is open, so this machine still cannot hibernate and no-hibernation remains the only honest configuration.
...
The kernel is right to refuse. Secret memory exists so that a key is unreadable outside the process that owns it; hibernating would write it to disk in the clear. On this machine the root filesystem was unencrypted when this was written, so that image would have sat in plaintext next to everything else — exactly the exposure the password manager is avoiding. The root filesystem is now encrypted (ADR-0009), so the exposure half of this paragraph no longer holds; the paragraph survives for the reasoning, which ADR-0009's note above preserves in full.
