...
A host declares only what nothing else can know: its hardware, hostname, keyboard layout, state version, and the boot entries specific to that machine. It imports feature modules for everything else. Shared policy lives in `base`, an ordinary feature module every host imports. Before this, the single host's `configuration.nix` was 150 lines of which roughly five were actually about that machine.
