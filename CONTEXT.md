...
**Host**:
A machine this flake builds a complete NixOS system for. A host declares only its identity: its hardware, hostname, keyboard layout, state version, and the settings nothing but that machine can know. It imports the feature modules that supply everything else.
...
**Authored dotfile**:
A configuration file a human writes, linked out of the store so an edit takes effect without a rebuild. Only authored dotfiles live in `.dotfiles`. Anything a running program writes, regenerated theme snippets or settings a UI panel saves, is runtime output, is not linked, and does not belong in this repo.
